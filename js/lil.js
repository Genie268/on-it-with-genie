...
async function renderChat(){
  ...
  const bubbles=messages.map((m,i)=>{
    ...
    const moreMenu=m.id?` <span onclick="event.stopPropagation();_toggleMsgMenu('${m.id}')" style="cursor:pointer;color:${isMe?"rgba(0,0,0,.25)":"#444"};font-size:14px;margin-left:4px;padding:2px;line-height:1;vertical-align:middle" onmouseenter="this.style.color='${isMe?"rgba(0,0,0,.5)":"#888"}'" onmouseleave="this.style.color='${isMe?"rgba(0,0,0,.25)":"#444'"}">⋮</span><div id="msg-menu-${m.id}" style="display:none;position:absolute;${isMe?"right:0":"left:0"};bottom:20px;background:#181818;border:1px solid #2a2a2a;border-radius:12px;box-shadow:0 8px 24px rgba(0,0,0,.6);overflow:hidden;z-index:100;min-width:140px"><button onclick="event.stopPropagation();chatSetReply('${m.id}','${msgPreview}');_closeMsgMenus()" style="display:flex;align-items:center;justify-content:space-between;width:100%;padding:11px 16px;background:none;border:none;border-bottom:1px solid #222;color:#ccc;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;text-align:left">Reply<span style="font-size:14px;color:#555">↩</span></button><button onclick="event.stopPropagation();_copyMsgText('${m.id}');_closeMsgMenus()" style="display:flex;align-items:center;justify-content:space-between;width:100%;padding:11px 16px;background:none;border:none;${isMe?"border-bottom:1px solid #222;":""}color:#ccc;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;text-align:left">Copy<span style="font-size:14px;color:#555">⊡</span></button>${isMe?`<button onclick="event.stopPropagation();unsendChatMsg('${m.id}');_closeMsgMenus()" style="display:flex;align-items:center;justify-content:space-between;width:100%;padding:11px 16px;background:none;border:none;color:#d9503a;font-size:13px;font-weight:600;cursor:pointer;font-family:inherit;text-align:left">Unsend<span style="font-size:14px">⊘</span></button>`:""}</div>`:"";
    return `${dateSep}<div id="msg-${m.id||i}" class="cmsg ${isMe?"cmsg-me":"cmsg-them"}" style="position:relative">
      <div class="cmsg-body">${body}</div>
      <div class="cmsg-time" style="position:relative">${isMe?"You":"Genie"} · ${timeStr}${readCheck}${moreMenu}</div>
    </div>`;
  }).join("");
